    ...
    <div className="space-y-4">
        {servicos.map((servico) => (
            <div key={servico.id} className="border rounded-lg p-4 bg-white dark:bg-gray-200 shadow-md">
                {/* Mapeando as categorias e exibindo o título de cada uma */}
                {servico.scategorias && servico.scategorias.length > 0 ? (
                    <div>
                        {servico.scategorias.map((scategoria) => (
                            <div key={scategoria.id} className="ml-4 text-lg font-bold text-orange-500">
                                {scategoria.titulo}
                            </div>
                        ))}
                    </div>
