...
const Paginicial = () => {
  ...
  // Função para buscar os dados do endereço baseado no endereco_id
  const fetchEndereco = async (enderecoId) => {
    const token = localStorage.getItem('token');
    try {
      const backendUrl = process.env.NEXT_PUBLIC_BACKEND_URL;
      const enderecoResponse = await axios.get(`${backendUrl}/api/endereco/${enderecoId}`, {
        headers: {
          Authorization: `Bearer ${token}`,
          'Content-Type': 'application/json',
        },
      });
      return enderecoResponse.data || null;
    } catch (error) {
  ...
  useEffect(() => {
    ...
    const fetchAnuncios = async () => {
      try {
        ...
        console.log(anunciosResponse.data.anuncios); // Verificando dados de anúncios
        
        if (anunciosResponse.data && Array.isArray(anunciosResponse.data.anuncios)) {
          const anunciosData = anunciosResponse.data.anuncios;

          // Agora buscamos o endereço de cada anúncio
          const anunciosComEndereco = await Promise.all(anunciosData.map(async (anuncio) => {
            const endereco = await fetchEndereco(anuncio.endereco_id);
            return {
              ...anuncio,
          ...
          setAnuncios(anunciosComEndereco);
        } else {
          setAnuncios([]);
            ...
            {loading ? (
              ...
              <div>
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                  {currentAnuncios.map(anuncio => (
                    <div key={anuncio.id} className="border rounded-lg p-4 shadow-md cursor-pointer" onClick={() => handleReservar(anuncio.id)}>
                      ...
                      <p className="text-gray-600">Cidade: {anuncio.cidade || 'Cidade não disponível'}</p>
                      <p className="text-gray-600">Bairro: {anuncio.bairro || 'Bairro não disponível'}</p>
                    </div>
                  ))}
                </div>
                {/* Paginação */}
                <div className="flex justify-center mt-6">
                  ...
                  <button
                    disabled={currentPage * anunciosPerPage >= anuncios.length}
                    onClick={() => setCurrentPage(currentPage + 1)}
